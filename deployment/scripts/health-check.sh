#!/usr/bin/env bash
# ====================================================================
# 微博情感分析系统  部署自检脚本 (精简版 7 容器)
# ====================================================================
# 检查: Docker / 端口 / 容器状态 / MySQL / HDFS / Spark / HTTP 接口
...
if [[ -f "${ENV_FILE}" ]]; then
    ...
    HDFS_NN_HTTP=$(get_env "HDFS_NAMENODE_HTTP_PORT" "9870")
    HDFS_DN_HTTP=$(get_env "HDFS_DATANODE_HTTP_PORT" "9864")
else
    WEB_PORT=5000; DB_PORT=3306; FRONTEND_PORT=3001
    SPARK_UI_PORT=8080; SPARK_PORT=7077
    HDFS_NN_PORT=9000; HDFS_NN_HTTP=9870; HDFS_DN_HTTP=9864
fi
...
CONTAINERS=(
    "weibo_db:MySQL"
    "weibo_web:Flask"
    "weibo_frontend:Frontend"
    "weibo_namenode:HDFS_NameNode"
    "weibo_datanode:HDFS_DataNode"
    "weibo_spark_master:SparkMaster"
    "weibo_spark_worker:SparkWorker"
)
...
if docker exec weibo_db mysqladmin ping -h localhost -u root -p"${DB_ROOT_PW}" &>/dev/null 2>&1; then
    pass "MySQL ping 成功"

    db_exists=$(docker exec weibo_db mysql -u root -p"${DB_ROOT_PW}" -e "SHOW DATABASES LIKE '${DB_NAME}';" 2>/dev/null | grep -c "${DB_NAME}" || echo "0")
    if [[ "${db_exists}" -gt 0 ]]; then
        pass "数据库 '${DB_NAME}' 存在"
        table_count=$(docker exec weibo_db mysql -u root -p"${DB_ROOT_PW}" -D "${DB_NAME}" -e "SHOW TABLES;" 2>/dev/null | wc -l || echo "0")
...
if [[ ${FAIL_COUNT} -eq 0 && ${WARN_COUNT} -eq 0 ]]; then
    ...
    echo -e "  ${YELLOW}${BOLD}  ${WARN_COUNT} 个警告，0 个错误。系统基本正常。${NC}"
else
