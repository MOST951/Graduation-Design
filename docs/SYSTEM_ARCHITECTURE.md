...
| NLP | hfl/chinese-bert-wwm-ext + jieba | 全词掩码中文BERT+传统分词 |
| 情感分析 | 词典+BERT级联 | 纯BERT Acc=87.79%, 级联 Acc=85.73% |
...
- **级联策略**: 词典置信度>θ(0.7)直出，否则调用BERT，词典直出17.2%、BERT处理82.8%
...
| 指标 | 目标值 | 当前值 |
|------|--------|--------|
| 纯BERT准确率 | >85% | 87.79% (Macro F1=0.8783) |
| 级联准确率 (θ=0.7) | >83% | 85.73% (Macro F1=0.8582) |
| 单条推理延迟 (bs=32) | <50ms | 24.04 ms/条 |
| 三维度 NDCG@10 | >0.85 | 0.9051 (提升46.4%) |
