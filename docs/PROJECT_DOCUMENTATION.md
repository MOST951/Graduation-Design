...
| 级联情感分析 | 词典优先+BERT精确分析（级联策略），准确率达85.73%，Macro F1=0.8582 |
...
对于词典方法难以处理的复杂文本（如含有反语、网络新词、长距离依赖等），本文采用微调后的 `hfl/chinese-bert-wwm-ext`（Whole Word Masking 中文 BERT）模型。该模型在BERT基础上采用全词掩码预训练策略，更擅长处理中文语义，对微博中的网络用语和表情符号具有更好的适应性。模型在公开微博情感数据集 weibo_senti_100k.csv（约10万条）上微调，训练参数为：max_length=192，batch_size=24，BERT lr=3e-5 + classifier lr=1e-4，epochs=3，warmup=0.05。微调后模型在10000条隔离测试集上的准确率达到 **87.79%**（Macro F1=0.8783），其中负面 F1=90.11%、正面 F1=89.35%、中性 F1=84.02%。对于输入文本 \(T\)，模型输出三类情感（正面、负面、中性）的概率，将其映射为情感得分 \(S_{\text{bert}}(T) \in [-1, 1]\)。
...
实验表明，当 θ=0.7 时，约 **17.2%** 的微博文本由词典直接输出，**82.8%** 调用BERT处理。在 CPU 环境下配置 batch_size=32，推理速度相比逐条推理提升 **3.10倍**（24.04 ms/条 vs 74.53 ms/条）。级联策略最终准确率达到 **85.73%**（Macro F1=0.8582），显著优于纯词典方法（Acc=63.59%，Macro F1=0.6265）。θ 在 [0.5, 0.8] 区间内指标完全一致，具有良好的鲁棒性。
...
基于 610 条真实 MySQL 微博数据的排序实验结果：

| 排序方法 | 权重配置 | NDCG@10 | NDCG@20 | 说明 |
|----------|----------|---------|---------|------|
| **本文三维度** | (0.4, 0.4, 0.2) | **0.9051** | **0.9184** | 推荐配置 |
| 网格搜索最优 | (0.1, 0.8, 0.1) | 1.0000 | - | 等价于仅看热度，违背设计初衷 |
| 等权三维 | (0.33, 0.33, 0.33) | 0.8830 | - | 本文提升 2.5% |
| 仅热度（传统） | (0, 1, 0) | 0.6181 | - | **本文提升 46.4%** |
...
**解决**:
1. 系统使用**级联策略**，约17.2%的文本由词典直接输出（毫秒级），82.8%调用BERT
...
| v3.0.0 | 2026-05-18 | **数据修正**：修正全部实验数据为真实值（纯BERT Acc=87.79%、级联 Acc=85.73%、NDCG@10=0.9051），补充批处理性能数据、三维度对比实验表格、模型名称修正为 hfl/chinese-bert-wwm-ext |
